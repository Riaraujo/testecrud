...
// Rotas para buscar dados únicos do banco de dados
app.get('/api/dados/disciplinas', async (req, res) => {
    try {
        const disciplinas = await Questao.distinct('disciplinas');
        res.json(disciplinas.filter(d => d && d.trim() !== ''));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

app.get('/api/dados/materias', async (req, res) => {
    try {
        const { disciplinas } = req.query;
        let filter = {};
        
        if (disciplinas) {
            const disciplinasArray = disciplinas.split(',');
            filter.disciplinas = { $in: disciplinasArray };
        }
        
        const materias = await Questao.distinct('materias', filter);
        res.json(materias.filter(m => m && m.trim() !== ''));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

app.get('/api/dados/assuntos', async (req, res) => {
    try {
        const { disciplinas, materias } = req.query;
        let filter = {};
        ...
        if (materias) {
            ...
            filter.materias = { $in: materiasArray };
        }
        
        const assuntos = await Questao.distinct('assuntos', filter);
        res.json(assuntos.filter(a => a && a.trim() !== ''));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

app.get('/api/dados/conteudos', async (req, res) => {
    try {
        const { disciplinas, materias, assuntos } = req.query;
        let filter = {};
        ...
        if (assuntos) {
            const assuntosArray = assuntos.split(',');
            filter.assuntos = { $in: assuntosArray };
        }
        
        const conteudos = await Questao.distinct('conteudos', filter);
        res.json(conteudos.filter(c => c && c.trim() !== ''));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

app.get('/api/dados/topicos', async (req, res) => {
    try {
        const { disciplinas, materias, assuntos, conteudos } = req.query;
        let filter = {};
        ...
        if (conteudos) {
            const conteudosArray = conteudos.split(',');
            filter.conteudos = { $in: conteudosArray };
        }
        
        const topicos = await Questao.distinct('topicos', filter);
        res.json(topicos.filter(t => t && t.trim() !== ''));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
